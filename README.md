...
*Основана* на теме [Minos](https://github.com/carsonip/hugo-theme-minos), а так же изменениях, внесённых [Дмитрием Ковалёвым](https://github.com/devpew) и используемых на [его сайте](https://devpew.com/). Ключевые настройки и дополнительные возможности темы перечислены ниже.
...
Все блоки с кодом будут содержать кнопку "копировать", по нажатию на которую содержимое соответствующего блока с кодом будет скопировано в буфер обмена. Вне зависимости от того, оформлены они в разметке Markdown, либо добавленные при помощи Hugo shortcodes.

Цвета для кнопки копирования взяты из палитры цветовой темы `gruvbox`. Рекомендуется установить её в качестве темы для блоков с кодом в конфиге:
