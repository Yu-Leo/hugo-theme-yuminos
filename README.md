...
**[Yuminos](https://github.com/Yu-Leo/hugo-theme-yuminos)** - минималистичная, но функциональная тема для генератора статических сайтов [Hugo](https://gohugo.io/).

*Основана* на теме [Minos](https://github.com/carsonip/hugo-theme-minos), а так же изменениях, внесённых [Дмитрием Ковалёвым](https://github.com/devpew) и используемых на [его сайте](https://devpew.com/). Возможности и настройки темы перечислены ниже.

## ❗Дисклеймер

1. Проект находится в процессе разработки. Может содержать недочёты и ошибки как в UI/UX, так и в фактической реализации задуманного: "костыли", не оптимальные решения, дублирование кода, некрасивый код и т. д. и т. п. [Issues](https://github.com/Yu-Leo/hugo-theme-yuminos/issues) с замечаниями и предложениями, а так же [Pull Requests](https://github.com/Yu-Leo/hugo-theme-yuminos/pulls) с исправлениями приветствуются!
...
3. Корректное отображение и функционирование блока с оглавлением постов (`table of content`, `toc`) при включении данной опции в конфиге **не гарантируется!**
...
- Дополненение [copy-tex](https://github.com/KaTeX/KaTeX/tree/main/contrib/copy-tex) заменяет отредеренные фрагменты на исходную TeX-верстку при выделении и копировании
- Поддерживаемые операции: https://katex.org/docs/supported.html
- Простой TeX редактор: https://latexeditor.lagrida.com/
### Блоки кода

Все блоки кода содержат кнопку "копировать", по нажатию на которую содержимое соответствующего блока копируется в буфер обмена. Вне зависимости от того, оформлены блоки кода в разметке Markdown, либо добавленные при помощи Hugo shortcodes.

Тема Yuminos подразумевает использование цветовой схемы `gruvbox`. Цвета для кнопки копирования взяты из её палитры. Тема содержит встроенные стили для блоков кода (hightlight-style.css), основанные на теме `gruvbox`.
...
Встроенные стили для блоков кода (hightlight-style.css) имеют кастомное оформление для языка `diff`:
...
Для улучшения SEO шаблоны темы содержат мета-теги. Значения параметров `title`, `description` и `keywords` берутся из параметров постов. При отсутствии в параметрах постов - из конфигурационного файла сайта.
...
Для каждого поста можно отдельно отключить комментарии его параметрах:
...
Присутствует интеграция с [Goat](https://github.com/blampe/goat).
